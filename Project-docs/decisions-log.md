...
| D24 | Mobile vs Desktop: niveles de funcionalidad | Mobile: categorización, listados, acceso a URLs, detección de episodios visual. NO síntesis LLM, NO automatización progresiva. Desktop: todo lo de mobile + síntesis LLM + exportación + automatización progresiva (State Machine completa). Mobile puede mostrar síntesis generadas previamente en desktop vía sync (lectura de SQLCipher), nunca generarlas. | La síntesis LLM requiere conectividad y capacidad de cómputo que no es adecuada para un flujo mobile. La diferenciación desktop/mobile es coherente con la plataforma primaria (D19) y evita complejizar el cliente Android. |
...
| D24 | Mobile vs Desktop: niveles de funcionalidad — síntesis solo en desktop | 2026-05-05 |
