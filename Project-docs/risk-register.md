...
| R21 | `tauri_plugin_shell::open` deprecado — pendiente migración a `tauri-plugin-opener` | warning en cada compilación; la API será eliminada en una release futura de Tauri 2.x | Technical Architect | sprint de mantenimiento cuando salga la versión estable de `tauri-plugin-opener`; hasta entonces no bloquea funcionalidad | Orchestrator | CONOCIDO — detectado durante HO-3-hardening-pre-beta (2026-05-06). No afecta a beta. Acción diferida: sustituir `app.shell().open()` por `tauri_plugin_opener` en `commands.rs:488` cuando la API sea estable. |
