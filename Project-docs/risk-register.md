...
| R14 | Workspace anticipado no se refresca con la app abierta | el desktop ya está abierto cuando llega un sync; el AnticipatedWorkspace no recalcula episodios automáticamente; el wow no dispara | Desktop Tauri Shell Specialist | añadir hook que recalcule clusters/episodes tras `mark_relay_acked` en estado de UI activo | Orchestrator | PARCIALMENTE MITIGADO — desktop: `relay-event-imported` emite y frontend recarga clusters/episodes. Confirmado en E2E 2026-04-30 (5/5 URLs). Mobile: fix `visibilitychange` en APK rebuild 2026-04-30, bajo observación en prueba de 7 días (H-001 evidencia que el APK está activo). |
